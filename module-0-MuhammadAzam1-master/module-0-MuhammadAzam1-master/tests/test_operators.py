...
def test_sigmoid(x):
    ...
    # TODO: Implement for Task 0.2.
    if x >= 0:
        assert_close(operators.sigmoid(x), 1.0 / (1.0 + math.exp(-x)))
    else:
        assert_close(operators.sigmoid(x), math.exp(x) / (1.0 + math.exp(x)))
...
def test_property(ls1, ls2):
    ...
    assert_close(sum(ls1) + sum(ls2), sum([i + j for i, j in zip(ls1, ls2)]))
