...
def shape_broadcast(shape1, shape2):
    ...
    """
    # TODO: Implement for Task 2.4.
    high = max(len(shape1), len(shape2))
    s1_r = list(reversed(shape1))
    s2_r = list(reversed(shape2))
    ans = [0] * high
    for i in range(high):
        if i >= len(shape1):
            ans[i] = s2_r[i]
            ...
            ans[i] = max(s1_r[i], s2_r[i])
            if s1_r[i] != ans[i] and s1_r[i] != 1 or s2_r[i] != ans[i] and s2_r[i] != 1:
