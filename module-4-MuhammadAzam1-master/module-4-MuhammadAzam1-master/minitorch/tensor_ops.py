...
def tensor_map(fn):
    ...
    def _map(out, out_shape, out_strides, in_storage, in_shape, in_strides):
        ...
        for i in range(len(out)):
            big = np.zeros(MAX_DIMS, np.int32)
            x = np.zeros(MAX_DIMS, np.int32)
            count(i, out_shape, big)
            broadcast_index(big, out_shape, in_shape, x)
            x_pos = index_to_position(x, in_strides)
            d = in_storage[x_pos]
            out[index_to_position(big, out_strides)] = fn(d)
...
def tensor_zip(fn):
    ...
    def _zip(
        ...
        for i in range(len(out)):
            ...
            count(i, out_shape, big)
            broadcast_index(big, out_shape, a_shape, a)
            broadcast_index(big, out_shape, b_shape, b)
            a_pos = index_to_position(a, a_strides)
            b_pos = index_to_position(b, b_strides)
            a_s = a_storage[a_pos]
            b_s = b_storage[b_pos]
            out[index_to_position(big, out_strides)] = fn(a_s, b_s)
