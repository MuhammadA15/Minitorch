    ...
    class Backend:
        ...
        class Sigmoid(Function):
            ...
            def backward(ctx, grad_output):
                ...
                return val * (-val + 1.0) * grad_output
        ...
        class Mean(Function):
            ...
            def forward(ctx, a, dim):
                # TODO: Implement for Task 2.2.
                if dim is None:
                    ctx.save_for_backward(a.size, dim, a.shape)
                    return add_reduce(a, list(range(a.dims))).view(1) / a.size
                else:
                    ctx.save_for_backward(a.shape[dim], dim, a.shape)
                    return add_reduce(a, [dim]) / a.shape[dim]

            @staticmethod
            def backward(ctx, grad_output):
                # TODO: Implement for Task 2.3.
                a, dim, a_shape = ctx.saved_values
                if dim is None:
                    out = grad_output.zeros(a_shape)
                    out._tensor._storage[:] = grad_output[0] / a
                    return out
                else:
        ...
        class MatMul(Function):
            ...
            def backward(ctx, grad_output):
                ...
                tp = list(range(t1.dims))
                tp[-2], tp[-1] = tp[-1], tp[-2]
                t1 = t1.permute(*tp)

                tp = list(range(t2.dims))
                tp[-2], tp[-1] = tp[-1], tp[-2]
                t2 = t2.permute(*tp)

                return (tensor_ops.matrix_multiply(grad_output, t2), tensor_ops.matrix_multiply(t1, grad_output),)
